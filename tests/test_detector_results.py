...
class DetectorResultTests(unittest.TestCase):
    def test_nonempty_results_in_global_and_local_inference(self):
        boxes = MagicMock()
        ...
        for names, expected_name in (({1: "standing"}, "standing"), (["sitting", "standing"], "standing"), ({}, "1")):
            for scope in ("global", "local"):
                with self.subTest(names=names, scope=scope):
                    result = SimpleNamespace(boxes=boxes, names=names)
                    detector._predict = Mock(return_value=[result])
                    detections = detector.detect(frame, scope=scope)
